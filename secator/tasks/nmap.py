...
class nmap(VulnMulti):
    ...
    opts = {
        ...
        TOP_PORTS: {
            "type": int,
            "short": "tp",
            "help": "Top ports to scan [full, 100, 1000]",
        },
    ...
    if platform.system() == "Darwin":
        install_cmd = (
            ...
            "sudo mkdir /opt/scipag_vulscan && sudo chown $USER /opt/scipag_vulscan && ls -l /opt; "
            "git clone https://github.com/scipag/vulscan /opt/scipag_vulscan; "
            "else "
            "cd /opt/scipag_vulscan && git pull; "
            "fi; "
            "if [ ! -L $(brew --prefix nmap)/share/nmap/scripts/vulscan ]; then "
        ...
        for host in self._get_hosts():
            ...
            for port in self._get_ports(host):
                ...
                for script in scripts:
                    ...
                    if not func:
                        debug(
                            f'Script output parser for "{script_id}" is not supported YET.',
                            sub="cve",
                        )
                        continue
    ...
    def _get_extra_data(self, port_cfg):
        ...
        if product and version:
            ...
            if cpe not in cpes:
                cpes.append(cpe)
                debug(
                    f"Added new CPE from identified product and version: {cpe}",
                    sub="cve",
                )
        ...
        for line in out.splitlines():
            ...
            if len(elems) == 4:  # exploit
                ...
                if vuln_type == "CVE" or vuln_type == "PRION:CVE":
                    ...
                    debug(
                        f'Vulners parser for "{vuln_type}" is not implemented YET.',
                        sub="cve",
                    )
            else:
