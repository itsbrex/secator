...
class cariddi(HttpCrawler):
    ...
    if platform.system() == "Darwin":
        install_cmd = (
            ...
            "if brew ls --versions cariddi >/dev/null 2>&1; then "
            "brew update >/dev/null 2>&1 && brew upgrade cariddi >/dev/null 2>&1 || true; "
            "else "
            "brew update >/dev/null 2>&1 && brew install cariddi >/dev/null 2>&1 || true; "
            "fi; "
            "else "
        ...
        with contextlib.suppress(json.decoder.JSONDecodeError):
            ...
            for param in params:
                ...
                for attack in param['attacks']:
                    ...
                    item = {'name': f'{attack} param', 'match': url, 'extra_data': extra_data}
                    items.append(item)

            for error in errors:
                match = error['match']
                match = f'{match[:1000]}...TRUNCATED' if len(match) > 1000 else match
