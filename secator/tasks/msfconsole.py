    ...
    def on_init(self):
        ...
        if command:
            self.run_opts["msfconsole.execute_command"] = (
                f"setg RHOST {self.input}; " f"setg RHOSTS {self.input}; " f"{command.format(**env_vars)}; " f"exit;"
            )
        ...
        # pass this temp file instead of the original one.
            ...
            logger.debug(f"Writing formatted resource script to new temp file {out_path}")
