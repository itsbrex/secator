...
class Report:
    ...
    """
    def __init__(self, runner, title=None, exporters=None):
    ...
    def as_table(self):
        print_results_table(self.data['results'], self.title)
    ...
    def build(self):
        ...
        for output_type in OUTPUT_TYPES:
            ...
            if items := [
                item
                for item in self.runner.results
                if isinstance(item, OutputType)
                and item._type == output_name
                and not item._duplicate
            ]:
